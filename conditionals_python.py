...
from allowed_functions import CONDITIONALS_ALLOWED
...
def expr_check(expr):
    ...
    if isinstance(expr, ast.BinOp):
        ...
        if isinstance(expr.op, ast.Not):
            return_val += "not " + expr_check(expr.operand)
        ...
        if func_name in CONDITIONALS_ALLOWED:
            # TODO: Modified function behaviors.
            # TODO: Deal with keywords.
            if len(expr.args) == 0:
                return_val = f"{func_name}()"
            else:
                args = [expr_check(arg) for arg in expr.args]
                return_val = f"{func_name}({', '.join(args)})"
        else:
            print("Call error!")
            raise ValueError(EXCEPTION_HELP)
...
def eval_node(node, indent_level):
    ...
    if isinstance(node, ast.Expr):
        ...
        current_line += "if(" + expr_check(node.test) + "):\n"
