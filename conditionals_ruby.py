...
def expr_check(expr):
    ...
    if isinstance(expr, ast.BinOp):
        ...
        if isinstance(expr.op, ast.Not):
            return_val += "not " + \
                "ruby_truth(" + expr_check(expr.operand) + ")"
        ...
        if expr.func.id == "help":
            ...
            return_val = "ruby_truth(" + expr_check(expr.args[0]) + ")"
        ...
        if isinstance(expr.op, ast.And):
            ...
            return_val += "ruby_and([" + ", ".join(values) + "])"
        elif isinstance(expr.op, ast.Or):
            return_val += "ruby_or([" + ", ".join(values) + "])"
        else:
            raise ValueError(EXCEPTION_HELP)
...
def eval_node(node, indent_level):
    ...
    # Broaden to Control Flow Stuff
        ...
        current_line += "if(" + \
            "ruby_truth(" + expr_check(node.test) + ")):\n"
